...
Active campaigns must live in the private campaigns repository, not inside Foundry-side modules.
...
- Foundry-side modules must not read campaign repositories directly and must not become the campaign source of truth.
...
- When preparing Foundry output for a campaign, run Uncle Yev's `npm run prep:foundry` with `FOUNDRY_CAMPAIGN_ROOT` pointed at the campaign folder. By convention it writes private generated output into `foundry-module/`, which should be installed in Foundry as the `uncle-yev` module.
